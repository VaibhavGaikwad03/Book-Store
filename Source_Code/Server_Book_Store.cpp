...
void BookStore::set_price()
{
    ...
    for (counter = 0; counter < BOOKLISTSIZE; counter++)
    {
        price.insert(make_pair(atoi(books_list[counter].substr(0, 5).c_str()), atoi(books_list[counter].substr(80, 5).c_str())));
    }
}
...
void BookStore::read_data(int category)
{
    ...
    while (getline(file, books_list[counter]))
        counter++;
...
void BookStore::display_books()
{
    ...
    for (counter = 0; counter < BOOKLISTSIZE; counter++)
    {
        display(books_list[counter].substr(0, 5), books_list[counter].substr(10, 40), books_list[counter].substr(50, 30), books_list[counter].substr(80, 5));
    }
}